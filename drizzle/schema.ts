...
export const outboundBoxes = mysqlTable("outbound_boxes", {
  id: int("id").autoincrement().primaryKey(),
  boxCode: varchar("boxCode", { length: 16 }).notNull().unique(),
  status: mysqlEnum("status", ["open", "sealed", "shipped"]).default("open").notNull(),
...
export type OutboundBox = typeof outboundBoxes.$inferSelect;
export type InsertOutboundBox = typeof outboundBoxes.$inferInsert;

/**
 * Product IDs generated by the transaction hub.
 * Stored as labelId for backward compatibility with existing code.
