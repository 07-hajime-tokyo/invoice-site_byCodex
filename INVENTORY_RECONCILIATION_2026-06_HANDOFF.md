...
## ツインパワー在庫・販売照合（2026-07-21）

`シマノ 21 ツインパワー SW 4000XG` の予備在庫について、仕入、eBay販売、出庫を照合した。

- 管理番号 `在庫0624_5`、モノタロウ仕入れ、59,378円、1台、6/30入庫。
- eBay注文 `24-14828-57860`、商品ID `366439375338`。eBay表示6/30 09:07 PDTは日本時間7/1 01:07で、7/6配達済み。
- 出庫No `ebay1702` で7/4 11:22に1台出庫し、元管理番号も一致。

したがって、59,378円は6/30棚卸在庫へ計上して正しく、現在庫は0。6月末在庫明細から削除しない。

未対応: eBay出品 `https://www.ebay.com/itm/366439375338` は確認時点で `Last one / Quantity 1` のまま表示されていた。別の実在庫がない場合は売り越し防止のため数量を0にするか出品を終了する。2026-07-21の確認では調査だけを行い、eBay、在庫サイト、Sheetは変更していない。
...
1. ツインパワーの別実在庫がないことを確認し、eBay商品ID `366439375338` の数量を0にするか出品を終了する。
2. N列を現時点の正本として使い、新しい修正メモが増えた場合はN列とN252を更新する。
3. DSi メタリックブルーの正しい商品名／カテゴリを、管理番号または現物で確認する。
4. 必要なら、N列の現在在庫金額を`6月末確定`タブのサマリーへ転記する。既存I列や上部固定値を置き換える場合は、従来値を消さず変更履歴を残す。
